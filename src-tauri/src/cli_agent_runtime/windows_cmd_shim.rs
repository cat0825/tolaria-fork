...
/// Volta manages CLIs through `.cmd` shims that re-dispatch via `volta run`,
/// e.g. `codex.cmd` containing `volta run "%~n0" %*`. Spawning that batch file
/// directly with complex agent arguments fails on Windows (`batch file
/// arguments are invalid`), so route through `volta run <tool> ...` instead.
fn volta_command_target(binary: &Path, contents: &str) -> Option<AgentCommandTarget> {
    if !contents.to_ascii_lowercase().contains("volta") {
        return None;
    }

    let tool = binary.file_stem()?.to_str()?;
    let volta = volta_program(binary, contents);

    Some(AgentCommandTarget {
        program: volta,
        prefix_args: vec![OsString::from("run"), OsString::from(tool)],
    })
}
